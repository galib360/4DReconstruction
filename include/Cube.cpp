...
const int signConfigToIntersectedEdges[256] = { 0x0, 0x109, 0x203, 0x30a, 0x406,
...
const Edge edges[12] = { { 1, 0, 1 }, // edge 0
		{ 2, 1, 2 }, // edge 1
		{ 4, 2, 3 }, // ...
		{ 8, 3, 0 }, { 16, 4, 5 }, { 32, 5, 6 }, { 64, 6, 7 }, { 128, 7, 4 }, {
				256, 0, 4 }, { 512, 1, 5 }, { 1024, 2, 6 }, { 2048, 3, 7 } // edge 11
};
}
...
Cube::Cube(Rect3 const& space, Fun3s const& sdf, cv::Mat const& voxel3D,
		Vec3 const& position) {
	auto mx = space.min.x;
	auto my = space.min.y;
	auto mz = space.min.z;
	...
	positions[1]={position.x+1, position.y, position.z};
	positions[2]={position.x+1, position.y, position.z+1};
	positions[3]={position.x, position.y, position.z+1};
	positions[4]={position.x, position.y+1, position.z};
	positions[5]={position.x+1, position.y+1, position.z};
	positions[6]={position.x+1, position.y+1, position.z+1};
	positions[7]={position.x, position.y+1, position.z+1};
	...
	for (auto i = 0; i < 8; ++i) {
		//need to put vert values here
		//auto sd = sdf(pos[i]);
		float sd = voxel3D.at<float>(positions[i].x, positions[i].y, positions[i].z);
		//std::cout<<sd<<std::endl;
		//std::cout << pos[i].x <<", "<<pos[i].y<<", "<<pos[i].z<< " value: "<<sd<<std::endl;
		if (sd == 0)
			sd += 1e-6;
		this->sdf[i] = sd;
	}
...
IntersectInfo Cube::Intersect(double iso) const {
	...
	for (auto e = 0; e < 12; ++e) {
		if (signConfigToIntersectedEdges[intersect.signConfig]
				& edges[e].edgeFlag) {
